...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysLogDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TSysLogDaoImpl:ITSysLogDao
    {
	  ...
	  #region 读操作
       ...
       public int GetCount(Expression<Func<TSysLog, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TSysLogs.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TSysLogs.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TSysLog, bool>> selector)
		//{
		//}
	...
	#region 获取所有的数据
       ...
       public List<TSysLog> GetList(Expression<Func<TSysLog, bool>> predicate)
            ...
            Expression<Func<TSysLog, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TSysLogs.Where(predicate);
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TSysLogs.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TSysLogs.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TSysLogs.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TSysLogs.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TSysLogs.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TSysLogs.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TSysLogs.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TSysLog字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TSysLog字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
