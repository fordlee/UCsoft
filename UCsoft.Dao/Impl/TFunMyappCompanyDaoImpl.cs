...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunMyappCompanyDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TFunMyappCompanyDaoImpl:ITFunMyappCompanyDao
    {
	  ...
	  #region 读操作
	   ...
	   #region  获取数据总数
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetCount(Expression<Func<TFunMyappCompany, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TFunMyappCompanies.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TFunMyappCompanies.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TFunMyappCompany, bool>> selector)
		//{
		//}
	...
	#region 获取所有的数据
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TFunMyappCompany> GetList(Expression<Func<TFunMyappCompany, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TFunMyappCompanies.Where<TFunMyappCompany>(predicate);
			   var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.ToList();
            }
		 ...
		 #region 查询分页
		  //查询分页
        public IPagination<TFunMyappCompany> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders,
            Expression<Func<TFunMyappCompany, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TFunMyappCompanies.Where(predicate);
		  ...
		  using (UCDbContext db=new UCDbContext())
            {
                //获取查询结果
                //加上扩展字段值
                var temps = db.TFunMyappCompanies;
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TFunMyappCompanies.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TFunMyappCompanies.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TFunMyappCompanies.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
       public   bool ExistsEntity(Expression<Func<TFunMyappCompany , bool>> predicate)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TFunMyappCompanies.Any(predicate);
               return status;
            }
        }
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TFunMyappCompanies.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
	  ...
	  //===========================================================================
	   /// <summary>
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
          using (UCDbContext db=new UCDbContext())
            {
               return db.DbHelper.ExecuteDataTable(sql,namedParameters).ToList<TFunMyappCompany>();
            }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TFunMyappCompanies.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TFunMyappCompanies.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TFunMyappCompanies.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TFunMyappCompany字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TFunMyappCompany字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
