...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现IVMyappCompanyDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class VMyappCompanyDaoImpl:IVMyappCompanyDao
    {
	     #region 读操作
	   ...
	   /// <summary>
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetViewCount(Expression<Func<VMyappCompany, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
       ...
       public List<VMyappCompany> GetViewList(Expression<Func<VMyappCompany, bool>> predicate)
        ...
        /// </summary>
        /// <returns></returns>
       public   bool ExistsViewEntity(Expression<Func<VMyappCompany , bool>> predicate)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.VMyappCompanies.Any(predicate);
               return status;
            }
        }
	   ...
	   //查询分页
        public IPagination<VMyappCompany> GetViewListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders,
            Expression<Func<VMyappCompany, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                 var roles = db.VMyappCompanies.Where(predicate);
