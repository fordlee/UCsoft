...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysPowerDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TSysPowerDaoImpl:ITSysPowerDao
    {
	  ...
	  #region 读操作
       ...
       public int GetCount(Expression<Func<TSysPower, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TSysPowers.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TSysPowers.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TSysPower, bool>> selector)
		//{
		//}
	...
	#region 获取所有的数据
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TSysPower> GetList(Expression<Func<TSysPower, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
		 ...
		 #region 查询分页
		  //查询分页
        public IPagination<TSysPower> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders,
            Expression<Func<TSysPower, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TSysPowers.Where(predicate);
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public TSysPower GetEntity(Expression<Func<TSysPower, bool>> predicate) 
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model =db.TSysPowers.Where<TSysPower>(predicate);
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TSysPowers.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TSysPowers.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TSysPowers.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TSysPowers.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
   ...
   #region 写操作
       ...
       /// <summary>
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TSysPowers.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TSysPower entity)
        {
            using (UCDbContext db=new UCDbContext())
            {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TSysPowers.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TSysPowers.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TSysPower字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TSysPower字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
