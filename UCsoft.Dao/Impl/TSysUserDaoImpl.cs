...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysUserDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TSysUserDaoImpl:ITSysUserDao
    {
	  ...
	  #region 读操作
       ...
       public int GetCount(Expression<Func<TSysUser, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TSysUsers.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TSysUsers.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TSysUser, bool>> selector)
		//{
		//}
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TSysUsers.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TSysUsers.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TSysUsers.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TSysUser字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TSysUser字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
