...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunAppCompanyDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TFunAppCompanyDaoImpl:ITFunAppCompanyDao
    {
	  ...
	  #region 读操作
       ...
       public int GetCount(Expression<Func<TFunAppCompany, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TFunAppCompanies.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TFunAppCompanies.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TFunAppCompany, bool>> selector)
		//{
		//}
	...
	#region 获取所有的数据
        ...
        public List<TFunAppCompany> GetList() 
        {
          using (UCDbContext db=new UCDbContext())
            {
             var models= db.TFunAppCompanies;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TFunAppCompany> GetList(Expression<Func<TFunAppCompany, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
		 ...
		 #region 查询分页
		  //查询分页
        public IPagination<TFunAppCompany> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders,
            Expression<Func<TFunAppCompany, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TFunAppCompanies.Where(predicate);
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public TFunAppCompany GetEntity(Expression<Func<TFunAppCompany, bool>> predicate) 
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model =db.TFunAppCompanies.Where<TFunAppCompany>(predicate);
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TFunAppCompanies.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TFunAppCompanies.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TFunAppCompanies.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TFunAppCompanies.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
	  ...
	  //===========================================================================
	   /// <summary>
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
          using (UCDbContext db=new UCDbContext())
            {
               return db.DbHelper.ExecuteDataTable(sql,namedParameters).ToList<TFunAppCompany>();
            }
   ...
   #region 写操作
       ...
       /// <summary>
        ...
        /// </summary>
         /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TFunAppCompany , bool>> predicate) 
        {
            using (UCDbContext db=new UCDbContext())
            {
                TFunAppCompany  entity = db.TFunAppCompanies.Where(predicate).First();
                int rows=db.TFunAppCompanies.Delete(entity);
				 if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TFunAppCompanies.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
                            db.TFunAppCompanies.Delete(item);
                         }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TFunAppCompany entity)
        {
            using (UCDbContext db=new UCDbContext())
            {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TFunAppCompanies.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TFunAppCompanies.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TFunAppCompany字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TFunAppCompany字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
