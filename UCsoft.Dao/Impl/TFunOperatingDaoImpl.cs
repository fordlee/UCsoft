...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunOperatingDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TFunOperatingDaoImpl:ITFunOperatingDao
    {
	  ...
	  #region 读操作
       ...
       public int GetCount(Expression<Func<TFunOperating, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TFunOperatings.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TFunOperatings.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TFunOperating, bool>> selector)
		//{
		//}
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TFunOperatings.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TFunOperatings.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TFunOperatings.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TFunOperatings.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TFunOperatings.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TFunOperatings.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TFunOperatings.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TFunOperating字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TFunOperating字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
