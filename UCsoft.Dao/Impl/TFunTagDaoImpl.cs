...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunTagDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TFunTagDaoImpl:ITFunTagDao
    {
	  ...
	  #region 读操作
	   ...
	   #region  获取数据总数
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetCount(Expression<Func<TFunTag, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TFunTags.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TFunTags.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TFunTag, bool>> selector)
		//{
		//}
	...
	#region 获取所有的数据
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TFunTag> GetList(Expression<Func<TFunTag, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TFunTags.Where<TFunTag>(predicate);
			   var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.ToList();
            }
		 ...
		 #region 查询分页
		  //查询分页
        public IPagination<TFunTag> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders,
            Expression<Func<TFunTag, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TFunTags.Where(predicate);
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public TFunTag GetEntity(Expression<Func<TFunTag, bool>> predicate) 
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model =db.TFunTags.Where<TFunTag>(predicate);
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TFunTags.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TFunTags.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TFunTags.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TFunTags.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
   ...
   #region 写操作
       ...
       /// <summary>
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TFunTags.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TFunTag entity)
        {
            using (UCDbContext db=new UCDbContext())
            {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TFunTags.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TFunTags.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TFunTag字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TFunTag字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
