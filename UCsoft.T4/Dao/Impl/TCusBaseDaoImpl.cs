...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITCusBaseDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TCusBaseDaoImpl:ITCusBaseDao
    {
	  ...
	  #region 读操作
       ...
       public int GetCount(Expression<Func<TCusBase, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TCusBases.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TCusBases.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TCusBase, bool>> selector)
		//{
		//}
	...
	#region 获取所有的数据
       ...
       public List<TCusBase> GetList(Expression<Func<TCusBase, bool>> predicate)
            ...
            Expression<Func<TCusBase, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TCusBases.Where(predicate);
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TCusBases.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TCusBases.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TCusBases.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TCusBases.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
