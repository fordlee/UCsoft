...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITCusLogDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TCusLogDaoImpl:ITCusLogDao
    {
	  ...
	  #region 读操作
       ...
       public int GetCount(Expression<Func<TCusLog, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TCusLogs.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TCusLogs.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TCusLog, bool>> selector)
		//{
		//}
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TCusLogs.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TCusLogs.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TCusLogs.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TCusLogs.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
   ...
   #region 写操作
       ...
       /// <summary>
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TCusLogs.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TCusLog entity)
        {
            using (UCDbContext db=new UCDbContext())
            {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TCusLogs.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TCusLogs.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TCusLog字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TCusLog字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
