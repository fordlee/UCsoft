...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysCompanyDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TSysCompanyDaoImpl:ITSysCompanyDao
    {
	  ...
	  #region 读操作
	   ...
	   #region  获取数据总数
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetCount(Expression<Func<TSysCompany, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TSysCompanies.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TSysCompanies.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TSysCompany, bool>> selector)
		//{
		//}
	...
	#region 获取所有的数据
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TSysCompany> GetList(Expression<Func<TSysCompany, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TSysCompanies.Where<TSysCompany>(predicate);
			   var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.ToList();
            }
		 ...
		 #region 查询分页
		  //查询分页
        public IPagination<TSysCompany> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders,
            Expression<Func<TSysCompany, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TSysCompanies.Where(predicate);
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public TSysCompany GetEntity(Expression<Func<TSysCompany, bool>> predicate) 
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model =db.TSysCompanies.Where<TSysCompany>(predicate);
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TSysCompanies.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TSysCompanies.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TSysCompanies.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TSysCompanies.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TSysCompanies.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TSysCompanies.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TSysCompanies.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TSysCompany字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TSysCompany字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
