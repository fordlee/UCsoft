...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysDepartmentDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TSysDepartmentDaoImpl:ITSysDepartmentDao
    {
	  ...
	  #region 读操作
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TSysDepartments.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TSysDepartment, bool>> selector)
		//{
		//}
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TSysDepartments.Where(field.Predicate, values).FirstOrDefault();
