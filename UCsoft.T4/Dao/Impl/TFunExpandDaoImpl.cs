...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunExpandDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TFunExpandDaoImpl:ITFunExpandDao
    {
	  ...
	  #region 读操作
       ...
       public int GetCount(Expression<Func<TFunExpand, bool>> predicate)
        {
             using (UCDbContext db=new UCDbContext())
            {
             var models= db.TFunExpands.Where(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
	   ...
	   using (UCDbContext db=new UCDbContext())
           {
               var models = db.TFunExpands.Max(temp => temp.Id);
               var sqlText = models.GetProperty("SqlText");
               LogHelper.Debug(sqlText.ToString());
               return models + 1;
           }
        ...
        /// </summary>
        /// <returns></returns>
        //public int GetMaxId(Expression<Func<TFunExpand, bool>> selector)
		//{
		//}
	...
	#region 获取所有的数据
       ...
       public List<TFunExpand> GetList(Expression<Func<TFunExpand, bool>> predicate)
            ...
            Expression<Func<TFunExpand, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TFunExpands.Where(predicate);
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TFunExpands.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TFunExpands.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TFunExpands.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TFunExpands.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
   ...
   #region 写操作
       ...
       /// <summary>
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TFunExpands.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TFunExpand entity)
        {
            using (UCDbContext db=new UCDbContext())
            {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TFunExpands.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TFunExpands.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TFunExpand字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TFunExpand字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
