...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunFilterDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class TFunFilterDaoImpl:ITFunFilterDao
	...
	#region 获取所有的数据
       ...
       public List<TFunFilter> GetList(Expression<Func<TFunFilter, bool>> predicate)
            ...
            Expression<Func<TFunFilter, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                var roles = db.TFunFilters.Where(predicate);
   ...
   public     List<Dictionary<string, object>> GetListWithExpByPage(int pageIndex, int pageSize, string selector,
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (UCDbContext db=new UCDbContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TFunFilters.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TFunFilters.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.TFunFilters.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (UCDbContext db=new UCDbContext())
            {
               bool status= db.TFunFilters.Where(predicate,values).Any();
               return status;
            }
        }
		 #endregion
   ...
   #region 写操作
       ...
       /// <summary>
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TFunFilters.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TFunFilter entity)
        {
            using (UCDbContext db=new UCDbContext())
            {
		 ...
		 using (UCDbContext db=new UCDbContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TFunFilters.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TFunFilters.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TFunFilter字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TFunFilter字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
