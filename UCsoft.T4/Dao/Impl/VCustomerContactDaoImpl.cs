...
namespace  UCsoft.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现IVCustomerContactDao接口的Dao类。 2014-11-05 07:48:35 By 唐有炜
    /// </summary>
 public class VCustomerContactDaoImpl:IVCustomerContactDao
    {
	     #region 读操作
       ...
       public List<VCustomerContact> GetViewList(Expression<Func<VCustomerContact, bool>> predicate)
        ...
        public IQueryable<Object> GetViewFields(string selector, string predicate)
        {
            using (UCDbContext db=new UCDbContext())
            {
                var model = db.VCustomerContacts.Where(predicate).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
	   ...
	   //查询分页
        public IPagination<VCustomerContact> GetViewListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders,
            Expression<Func<VCustomerContact, bool>> predicate)
        {
            using (UCDbContext db = new UCDbContext())
            {
                 var roles = db.VCustomerContacts.Where(predicate);
	  ...
	  //===========================================================================
	   /// <summary>
        ...
        public DataTable GetViewListBySql(string sql, dynamic namedParameters)
        {
          using (UCDbContext db=new UCDbContext())
            {
               return db.DbHelper.ExecuteDataTable(sql,namedParameters).ToList<VCustomerContact>();
            }
