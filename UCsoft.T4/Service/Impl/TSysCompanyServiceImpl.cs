...
{
public  class TSysCompanyServiceImpl: ITSysCompanyService
    {
	
	#region  依赖注入 2014-11-05 20:23:33 By 唐有炜
	public ITSysCompanyDao  TSysCompanyDao{set;get;}
	#endregion
        ...
        public IEnumerable<TSysCompany> GetSysCompanyList()
		{
		  try
            {
                var models = TSysCompanyDao.GetList();
                LogHelper.Debug("获取SysCompany所有数据成功。");
                return models;
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取SysCompany所有数据失败。", ex);
                return null;
            }
		}
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysCompany> GetSysCompanyList( Expression<Func<TSysCompany, bool>> predicate)
		{
		  try
            {
                var models = TSysCompanyDao.GetList(predicate);
                LogHelper.Debug("获取SysCompany所有数据成功。");
                return models;
            }
            catch (Exception ex)
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysCompany> GetSysCompanyList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TSysCompany, bool>> predicate,IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders)
        {
            try
            {
                var models = TSysCompanyDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
                LogHelper.Debug("获取SysCompany分页列表成功。");
                return models;
            }
            catch (Exception ex)
            {
                recordCount = 0;
                LogHelper.Error("获取SysCompany分页列表失败。", ex);
                return null;
            }
        }
       ...
       public DataTable GetSysCompanyList(string[] selectFields, int pageIndex, int pageSize, out int recordCount,
			...
			try
                {
					...
					//这里添加参数
                    var models = TSysCompanyDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetSysCompanyList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
            out int recordCount, params object[] values)
			{
		 try
                {
                var models = TSysCompanyDao.GetListByPage(pageIndex, pageSize, selector, predicate,
       ...
       public List<Dictionary<string, object>> GetSysCompanyList( string selector, string expFields, string expSelector,int pageIndex, int pageSize, out int recordCount,string predicate, string ordering,params object[] values)
	   {
	  
	   try
                {
                var models = TSysCompanyDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取SysCompany分页列表成功。");
                return models;
                }
	   ...
	   #region 查看
        ...
        /// </summary>
        /// <param name="predicate">查询条件</param>
        /// <returns>TSysCompany</returns>
       public TSysCompany GetSysCompany(Expression<Func<TSysCompany, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  TSysCompanyDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	...
	try
            {
                var fields =  TSysCompanyDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetSysCompany(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = TSysCompanyDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
        ...
        public  bool AddSysCompany(TSysCompany SysCompany)
		{
		 var status = false;
		       try
                {
                     status = TSysCompanyDao.InsertEntity(SysCompany);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysCompany失败。", ex);
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  TSysCompanyDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteSysCompany(string ids)
		{
		       var status = false;
		       try
                {
                ...
                status = TSysCompanyDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysCompany失败。", ex);
        ...
        {
            var status = TSysCompanyDao.ExistsEntity(predicate,values);
            return status;
        }
