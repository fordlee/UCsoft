...
{
public  class TCusBaseServiceImpl: ITCusBaseService
    {
	
	#region  依赖注入 2014-11-05 20:23:33 By 唐有炜
	public ITCusBaseDao  TCusBaseDao{set;get;}
	#endregion
        ...
        public IEnumerable<TCusBase> GetCusBaseList()
		{
		  try
            {
                var models = TCusBaseDao.GetList();
                LogHelper.Debug("获取CusBase所有数据成功。");
                return models;
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取CusBase所有数据失败。", ex);
                return null;
            }
		}
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TCusBase> GetCusBaseList( Expression<Func<TCusBase, bool>> predicate)
		{
		  try
            {
                var models = TCusBaseDao.GetList(predicate);
                LogHelper.Debug("获取CusBase所有数据成功。");
                return models;
            }
            catch (Exception ex)
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TCusBase> GetCusBaseList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TCusBase, bool>> predicate,IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders)
        {
            try
            {
                var models = TCusBaseDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
                LogHelper.Debug("获取CusBase分页列表成功。");
                return models;
            }
            catch (Exception ex)
            {
                recordCount = 0;
                LogHelper.Error("获取CusBase分页列表失败。", ex);
                return null;
            }
        }
       ...
       public DataTable GetCusBaseList(string[] selectFields, int pageIndex, int pageSize, out int recordCount,
			...
			try
                {
					...
					//这里添加参数
                    var models = TCusBaseDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetCusBaseList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
            out int recordCount, params object[] values)
			{
		 try
                {
                var models = TCusBaseDao.GetListByPage(pageIndex, pageSize, selector, predicate,
       ...
       public List<Dictionary<string, object>> GetCusBaseList( string selector, string expFields, string expSelector,int pageIndex, int pageSize, out int recordCount,string predicate, string ordering,params object[] values)
	   {
	  
	   try
                {
                var models = TCusBaseDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取CusBase分页列表成功。");
                return models;
                }
	   ...
	   #region 查看
        ...
        /// </summary>
        /// <param name="predicate">查询条件</param>
        /// <returns>TCusBase</returns>
       public TCusBase GetCusBase(Expression<Func<TCusBase, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  TCusBaseDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	...
	try
            {
                var fields =  TCusBaseDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetCusBase(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = TCusBaseDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
        ...
        public  bool AddCusBase(TCusBase CusBase)
		{
		 var status = false;
		       try
                {
                     status = TCusBaseDao.InsertEntity(CusBase);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取CusBase失败。", ex);
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  TCusBaseDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteCusBase(string ids)
		{
		       var status = false;
		       try
                {
                ...
                status = TCusBaseDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取CusBase失败。", ex);
        ...
        {
            var status = TCusBaseDao.ExistsEntity(predicate,values);
            return status;
        }
