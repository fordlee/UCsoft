...
{
public  class TCusConServiceImpl: ITCusConService
    {
	
	#region  依赖注入 2014-11-05 20:23:33 By 唐有炜
	public ITCusConDao  TCusConDao{set;get;}
	#endregion
        ...
        public IEnumerable<TCusCon> GetCusConList()
		{
		  try
            {
                var models = TCusConDao.GetList();
                LogHelper.Debug("获取CusCon所有数据成功。");
                return models;
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取CusCon所有数据失败。", ex);
                return null;
            }
		}
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TCusCon> GetCusConList( Expression<Func<TCusCon, bool>> predicate)
		{
		  try
            {
                var models = TCusConDao.GetList(predicate);
                LogHelper.Debug("获取CusCon所有数据成功。");
                return models;
            }
            catch (Exception ex)
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TCusCon> GetCusConList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TCusCon, bool>> predicate,IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders)
        {
            try
            {
                var models = TCusConDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
                LogHelper.Debug("获取CusCon分页列表成功。");
                return models;
            }
            catch (Exception ex)
            {
                recordCount = 0;
                LogHelper.Error("获取CusCon分页列表失败。", ex);
                return null;
            }
        }
       ...
       public DataTable GetCusConList(string[] selectFields, int pageIndex, int pageSize, out int recordCount,
			...
			try
                {
					...
					//这里添加参数
                    var models = TCusConDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetCusConList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
            out int recordCount, params object[] values)
			{
		 try
                {
                var models = TCusConDao.GetListByPage(pageIndex, pageSize, selector, predicate,
       ...
       public List<Dictionary<string, object>> GetCusConList( string selector, string expFields, string expSelector,int pageIndex, int pageSize, out int recordCount,string predicate, string ordering,params object[] values)
	   {
	  
	   try
                {
                var models = TCusConDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取CusCon分页列表成功。");
                return models;
                }
       ...
       public TCusCon GetCusCon(Expression<Func<TCusCon, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  TCusConDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	   	          LogHelper.Error("获取CusCon失败。", ex);
	   	          return null;
	   	      }
	   }
	...
	try
            {
                var fields =  TCusConDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetCusCon(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = TCusConDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
        ...
        /// </summary>
        /// <param name="TCusCon">TCusCon</param>
        /// <returns>添加状态</returns>
        public  bool AddCusCon(TCusCon CusCon)
		{
		 var status = false;
		       try
                {
                     status = TCusConDao.InsertEntity(CusCon);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取CusCon失败。", ex);
        ...
        /// <param name="id">id</param>
         /// <param name="TCusCon">TCusCon</param>
        /// <returns>添加状态</returns>
        public bool EditCusCon(TCusCon CusCon)
		{
		
		 var status = false;
		       try
                {
                     status = TCusConDao.UpdateEntity(CusCon);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取CusCon失败。", ex);
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  TCusConDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteCusCon(string ids)
		{
		       var status = false;
		       try
                {
                ...
                status = TCusConDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取CusCon失败。", ex);
        ...
        {
            var status = TCusConDao.ExistsEntity(predicate,values);
            return status;
        }
