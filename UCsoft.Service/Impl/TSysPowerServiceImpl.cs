...
{
public  class TSysPowerServiceImpl: ITSysPowerService
    {
	
	#region  依赖注入 2014-11-05 20:23:33 By 唐有炜
	public ITSysPowerDao  TSysPowerDao{set;get;}
	#endregion
        ...
        public IEnumerable<TSysPower> GetSysPowerList()
		{
		  try
            {
                var models = TSysPowerDao.GetList();
                LogHelper.Debug("获取SysPower所有数据成功。");
                return models;
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取SysPower所有数据失败。", ex);
                return null;
            }
		}
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysPower> GetSysPowerList( Expression<Func<TSysPower, bool>> predicate)
		{
		  try
            {
                var models = TSysPowerDao.GetList(predicate);
                LogHelper.Debug("获取SysPower所有数据成功。");
                return models;
            }
            catch (Exception ex)
        ...
        public IEnumerable<TSysPower> GetSysPowerList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TSysPower, bool>> predicate,IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders)
        {
            try
            {
                var models = TSysPowerDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
			...
			try
                {
					...
					//这里添加参数
                    var models = TSysPowerDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetSysPowerList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
            out int recordCount, params object[] values)
			{
		 try
                {
                var models = TSysPowerDao.GetListByPage(pageIndex, pageSize, selector, predicate,
       ...
       public List<Dictionary<string, object>> GetSysPowerList( string selector, string expFields, string expSelector,int pageIndex, int pageSize, out int recordCount,string predicate, string ordering,params object[] values)
	   {
	  
	   try
                {
                var models = TSysPowerDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取SysPower分页列表成功。");
                return models;
                }
       ...
       public TSysPower GetSysPower(Expression<Func<TSysPower, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  TSysPowerDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	...
	try
            {
                var fields =  TSysPowerDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetSysPower(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = TSysPowerDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
        ...
        public  bool AddSysPower(TSysPower SysPower)
		{
		 var status = false;
		       try
                {
                     status = TSysPowerDao.InsertEntity(SysPower);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysPower失败。", ex);
        ...
        public bool EditSysPower(TSysPower SysPower)
		{
		 ...
		 var status = false;
		       try
                {
                     status = TSysPowerDao.UpdateEntity(SysPower);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysPower失败。", ex);
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  TSysPowerDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteSysPower(string ids)
		{
		       ...
		       try
                ...
                status = TSysPowerDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysPower失败。", ex);
        ...
        {
            var status = TSysPowerDao.ExistsEntity(predicate,values);
            return status;
        }
