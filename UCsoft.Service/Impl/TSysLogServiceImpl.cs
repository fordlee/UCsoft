...
{
public  class TSysLogServiceImpl: ITSysLogService
    {
	...
	public ITSysLogDao  TSysLogDao{set;get;}
        ...
        public IEnumerable<TSysLog> GetSysLogList()
		{
		  try
            {
                var models = TSysLogDao.GetList();
                LogHelper.Debug("获取SysLog所有数据成功。");
                return models;
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取SysLog所有数据失败。", ex);
                return null;
            }
		}
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysLog> GetSysLogList( Expression<Func<TSysLog, bool>> predicate)
		{
		  try
            {
                var models = TSysLogDao.GetList(predicate);
                LogHelper.Debug("获取SysLog所有数据成功。");
                return models;
            }
            catch (Exception ex)
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysLog> GetSysLogList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TSysLog, bool>> predicate,IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders)
        {
            try
            {
                var models = TSysLogDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
                LogHelper.Debug("获取SysLog分页列表成功。");
                return models;
            }
            catch (Exception ex)
            {
                recordCount = 0;
                LogHelper.Error("获取SysLog分页列表失败。", ex);
                return null;
            }
        }
       ...
       public DataTable GetSysLogList(string[] selectFields, int pageIndex, int pageSize, out int recordCount,
			...
			try
                {
					...
					//这里添加参数
                    var models = TSysLogDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetSysLogList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
            out int recordCount, params object[] values)
			{
		 try
                {
                var models = TSysLogDao.GetListByPage(pageIndex, pageSize, selector, predicate,
       ...
       public List<Dictionary<string, object>> GetSysLogList( string selector, string expFields, string expSelector,int pageIndex, int pageSize, out int recordCount,string predicate, string ordering,params object[] values)
	   {
	  
	   try
                {
                var models = TSysLogDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取SysLog分页列表成功。");
                return models;
                }
       ...
       public TSysLog GetSysLog(Expression<Func<TSysLog, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  TSysLogDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	...
	try
            {
                var fields =  TSysLogDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetSysLog(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = TSysLogDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  TSysLogDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteSysLog(string ids)
		{
		       var status = false;
		       try
                {
                ...
                status = TSysLogDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysLog失败。", ex);
        ...
        {
            var status = TSysLogDao.ExistsEntity(predicate,values);
            return status;
        }
