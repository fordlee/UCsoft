...
{
public  class TSysUserServiceImpl: ITSysUserService
    {
	...
	public ITSysUserDao  TSysUserDao{set;get;}
        ...
        public IEnumerable<TSysUser> GetSysUserList()
		{
		  try
            {
                var models = TSysUserDao.GetList();
                LogHelper.Debug("获取SysUser所有数据成功。");
                return models;
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取SysUser所有数据失败。", ex);
                return null;
            }
		}
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysUser> GetSysUserList( Expression<Func<TSysUser, bool>> predicate)
		{
		  try
            {
                var models = TSysUserDao.GetList(predicate);
                LogHelper.Debug("获取SysUser所有数据成功。");
                return models;
            }
            catch (Exception ex)
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysUser> GetSysUserList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TSysUser, bool>> predicate,IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders)
        {
            try
            {
                var models = TSysUserDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
                LogHelper.Debug("获取SysUser分页列表成功。");
                return models;
            }
            catch (Exception ex)
            {
                recordCount = 0;
                LogHelper.Error("获取SysUser分页列表失败。", ex);
                return null;
            }
        }
       ...
       public DataTable GetSysUserList(string[] selectFields, int pageIndex, int pageSize, out int recordCount,
			...
			try
                {
					...
					//这里添加参数
                    var models = TSysUserDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetSysUserList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
            out int recordCount, params object[] values)
			{
		 try
                {
                var models = TSysUserDao.GetListByPage(pageIndex, pageSize, selector, predicate,
       ...
       public List<Dictionary<string, object>> GetSysUserList( string selector, string expFields, string expSelector,int pageIndex, int pageSize, out int recordCount,string predicate, string ordering,params object[] values)
	   {
	  
	   try
                {
                var models = TSysUserDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取SysUser分页列表成功。");
                return models;
                }
       ...
       public TSysUser GetSysUser(Expression<Func<TSysUser, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  TSysUserDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	   	          LogHelper.Error("获取SysUser失败。", ex);
	   	          return null;
	   	      }
	   }
	...
	try
            {
                var fields =  TSysUserDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetSysUser(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = TSysUserDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
        ...
        /// </summary>
        /// <param name="TSysUser">TSysUser</param>
        /// <returns>添加状态</returns>
        public  bool AddSysUser(TSysUser SysUser)
		{
		 var status = false;
		       try
                {
                     status = TSysUserDao.InsertEntity(SysUser);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysUser失败。", ex);
        ...
        /// <param name="id">id</param>
         /// <param name="TSysUser">TSysUser</param>
        /// <returns>添加状态</returns>
        public bool EditSysUser(TSysUser SysUser)
		{
		
		 var status = false;
		       try
                {
                     status = TSysUserDao.UpdateEntity(SysUser);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysUser失败。", ex);
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  TSysUserDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteSysUser(string ids)
		{
		       var status = false;
		       try
                {
                ...
                status = TSysUserDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysUser失败。", ex);
        ...
        {
            var status = TSysUserDao.ExistsEntity(predicate,values);
            return status;
        }
