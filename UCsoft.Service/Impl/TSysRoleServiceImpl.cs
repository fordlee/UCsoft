...
{
public  class TSysRoleServiceImpl: ITSysRoleService
    {
	...
	public ITSysRoleDao  TSysRoleDao{set;get;}
        ...
        public IEnumerable<TSysRole> GetSysRoleList()
		{
		  try
            {
                var models = TSysRoleDao.GetList();
                LogHelper.Debug("获取SysRole所有数据成功。");
                return models;
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取SysRole所有数据失败。", ex);
                return null;
            }
		}
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysRole> GetSysRoleList( Expression<Func<TSysRole, bool>> predicate)
		{
		  try
            {
                var models = TSysRoleDao.GetList(predicate);
                LogHelper.Debug("获取SysRole所有数据成功。");
                return models;
            }
            catch (Exception ex)
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TSysRole> GetSysRoleList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TSysRole, bool>> predicate,IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders)
        {
            try
            {
                var models = TSysRoleDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
                LogHelper.Debug("获取SysRole分页列表成功。");
                return models;
            }
            catch (Exception ex)
            {
                recordCount = 0;
                LogHelper.Error("获取SysRole分页列表失败。", ex);
                return null;
            }
        }
       ...
       public DataTable GetSysRoleList(string[] selectFields, int pageIndex, int pageSize, out int recordCount,
			...
			try
                {
					...
					//这里添加参数
                    var models = TSysRoleDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetSysRoleList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
            out int recordCount, params object[] values)
			{
		 try
                {
                var models = TSysRoleDao.GetListByPage(pageIndex, pageSize, selector, predicate,
       ...
       public List<Dictionary<string, object>> GetSysRoleList( string selector, string expFields, string expSelector,int pageIndex, int pageSize, out int recordCount,string predicate, string ordering,params object[] values)
	   {
	  
	   try
                {
                var models = TSysRoleDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取SysRole分页列表成功。");
                return models;
                }
       ...
       public TSysRole GetSysRole(Expression<Func<TSysRole, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  TSysRoleDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	   	          LogHelper.Error("获取SysRole失败。", ex);
	   	          return null;
	   	      }
	   }
	...
	try
            {
                var fields =  TSysRoleDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetSysRole(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = TSysRoleDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
        ...
        /// </summary>
        /// <param name="TSysRole">TSysRole</param>
        /// <returns>添加状态</returns>
        public  bool AddSysRole(TSysRole SysRole)
		{
		 var status = false;
		       try
                {
                     status = TSysRoleDao.InsertEntity(SysRole);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysRole失败。", ex);
        ...
        /// <param name="id">id</param>
         /// <param name="TSysRole">TSysRole</param>
        /// <returns>添加状态</returns>
        public bool EditSysRole(TSysRole SysRole)
		{
		
		 var status = false;
		       try
                {
                     status = TSysRoleDao.UpdateEntity(SysRole);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysRole失败。", ex);
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  TSysRoleDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteSysRole(string ids)
		{
		       var status = false;
		       try
                {
                ...
                status = TSysRoleDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysRole失败。", ex);
        ...
        {
            var status = TSysRoleDao.ExistsEntity(predicate,values);
            return status;
        }
