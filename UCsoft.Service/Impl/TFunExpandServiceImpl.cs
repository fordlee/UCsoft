...
{
public  class TFunExpandServiceImpl: ITFunExpandService
    {
	
	#region  依赖注入 2014-11-05 20:23:33 By 唐有炜
	public ITFunExpandDao  TFunExpandDao{set;get;}
	#endregion
        ...
        public IEnumerable<TFunExpand> GetFunExpandList()
		{
		  try
            {
                var models = TFunExpandDao.GetList();
                LogHelper.Debug("获取FunExpand所有数据成功。");
                return models;
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取FunExpand所有数据失败。", ex);
                return null;
            }
		}
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TFunExpand> GetFunExpandList( Expression<Func<TFunExpand, bool>> predicate)
		{
		  try
            {
                var models = TFunExpandDao.GetList(predicate);
                LogHelper.Debug("获取FunExpand所有数据成功。");
                return models;
            }
            catch (Exception ex)
        ...
        /// <param name="orders">排序字段（字段字典<字段、排序类型>）</param>
        /// <param name="predicate">筛选条件</param>
        public IEnumerable<TFunExpand> GetFunExpandList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TFunExpand, bool>> predicate,IDictionary<string, UCsoft.Entity.UCEnums.OrderEmum> orders)
        {
            try
            {
                var models = TFunExpandDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
                LogHelper.Debug("获取FunExpand分页列表成功。");
                return models;
            }
            catch (Exception ex)
            {
                recordCount = 0;
                LogHelper.Error("获取FunExpand分页列表失败。", ex);
                return null;
            }
        }
       ...
       public DataTable GetFunExpandList(string[] selectFields, int pageIndex, int pageSize, out int recordCount,
			...
			try
                {
					...
					//这里添加参数
                    var models = TFunExpandDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetFunExpandList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
            out int recordCount, params object[] values)
			{
		 try
                {
                var models = TFunExpandDao.GetListByPage(pageIndex, pageSize, selector, predicate,
       ...
       public List<Dictionary<string, object>> GetFunExpandList( string selector, string expFields, string expSelector,int pageIndex, int pageSize, out int recordCount,string predicate, string ordering,params object[] values)
	   {
	  
	   try
                {
                var models = TFunExpandDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取FunExpand分页列表成功。");
                return models;
                }
	   ...
	   #region 查看
        ...
        /// </summary>
        /// <param name="predicate">查询条件</param>
        /// <returns>TFunExpand</returns>
       public TFunExpand GetFunExpand(Expression<Func<TFunExpand, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  TFunExpandDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	...
	try
            {
                var fields =  TFunExpandDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetFunExpand(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = TFunExpandDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
        ...
        public  bool AddFunExpand(TFunExpand FunExpand)
		{
		 var status = false;
		       try
                {
                     status = TFunExpandDao.InsertEntity(FunExpand);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取FunExpand失败。", ex);
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  TFunExpandDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteFunExpand(string ids)
		{
		       var status = false;
		       try
                {
                ...
                status = TFunExpandDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取FunExpand失败。", ex);
        ...
        {
            var status = TFunExpandDao.ExistsEntity(predicate,values);
            return status;
        }
